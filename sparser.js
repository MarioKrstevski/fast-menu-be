...
const fetch = isBrowser
  ? /* istanbul ignore next */ window.fetch
  : require("../src/fetch");
...
class PublicGoogleSheetsParser {
  ...
  applyHeaderIntoRows(header, rows) {
    return rows
      ...
      .map((row) =>
        row.reduce(
          (p, c, i) =>
            c.v ? Object.assign(p, { [header[i]]: c.v }) : p,
          {}
        )
      );
  }
  ...
  getItems(spreadsheetResponse) {
    ...
    try {
      ...
      if (hasSomeLabelPropertyInCols) {
        ...
        rows = this.applyHeaderIntoRows(
          header,
          parsedJSON.table.rows
        );
      } else {
        const [headerRow, ...originalRows] = parsedJSON.table.rows;
        const header = this.normalizeRow(headerRow.c).map(
          (row) => row.v
        );
  ...
  async parse(spreadsheetId, sheetInfo) {
    ...
    const spreadsheetResponse =
      await this.getSpreadsheetDataUsingFetch();
